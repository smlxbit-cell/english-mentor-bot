...
async def _refresh_rules_bank_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if page_cb.startswith('rules:bank:topic:'):
        ...
        if parsed:
            level, category, _topic_idx, page = parsed
            await _show_rules_bank_page(
...
async def _show_word_bank_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_level = profile.get('level_code') or profile.get('cefr_level') or 'a1'
    text = (
        '📖 <b>Словарь · по уровню</b>\n\n'
        f'Ваш уровень: <b>{user_level.upper()}</b>\n'
        'Выберите уровень → слово → <b>Знаю</b> / <b>Учить</b>.'
    )
    await _send(
        context, _chat_id(update), text,
...
async def _show_bank_entry(update, context, bank_entry_id: int):
    ...
    await _play_tts(context, _chat_id(update), context.user_data['tts_text'])
...
async def _start_word_drill(
    ...
    *,
    ...
    context.user_data.pop('drill_context_order', None)
    context.user_data.pop('drill_context_index', None)
    context.user_data.pop('drill_context_example', None)
    context.user_data.pop('drill_context_example_ru', None)
...
async def _flash_drill_options_hint(
    context,
    chat_id: int,
    options: list[dict],
    *,
    ...
    from learning.word_bank.drill import format_option_translations_hint

    hint = await context.bot.send_message(
        chat_id,
        format_option_translations_hint(options),
        parse_mode=ParseMode.HTML,
    )
    await asyncio.sleep(seconds)
    try:
...
async def _show_drill_context_step(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    header: str,
    ...
    from learning.word_bank.drill import (
        ...
        format_drill_context_prompt,
        option_button_label,
        prepare_context_drill,
    )
    ...
    context.user_data['tts_text'] = ctx['tts']
    await _send(
        context, chat_id,
        format_drill_context_prompt(
            header,
            gap_sentence=_esc(ctx['gap_sentence']),
            example_ru=_esc(ctx['example_ru']),
        ),
        reply_markup=keyboards.word_drill_choice_kb(labels, step='context'),
        parse_mode=ParseMode.HTML,
    )
    await _play_tts(context, chat_id, ctx['tts'])
...
async def _show_drill_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    word_pos = drill_progress_pos(
        ...
        context_index=int(context.user_data.get('drill_context_index', 0)),
    )
    ...
    if step == 'context':
        await _show_drill_context_step(
            update, context,
            header=header, word=word, pool=pool, chat_id=chat_id,
        )
        return
...
def _advance_drill(context) -> bool | None:
    ...
    nxt = advance_drill(
        ...
        context_index=int(context.user_data.get('drill_context_index', 0)),
        context_order=context.user_data.get('drill_context_order'),
    )
    if nxt is None:
        return None
    ...
    context.user_data['drill_context_index'] = nxt['context_index']
    if nxt.get('listening_order') is not None:
        context.user_data['drill_listening_order'] = nxt['listening_order']
    if nxt.get('context_order') is not None:
        context.user_data['drill_context_order'] = nxt['context_order']
    return True
...
async def _handle_drill_pick(
    ...
    *,
    ...
    from learning.word_bank.drill import (
        ...
        format_choice_wrong,
        format_context_correct,
        format_context_wrong,
    ...
    if option_idx == correct_idx:
        if step == 'context':
            msg = format_context_correct(
                word=word,
                example=context.user_data.get('drill_context_example') or '',
                example_ru=context.user_data.get('drill_context_example_ru') or '',
            )
        else:
            msg = format_choice_correct(word)
        is_correct = True
    elif step == 'english' or step == 'context':
        ...
        if step == 'context':
            msg = format_context_wrong(
                picked=picked,
                correct=word,
                options=options_words,
                example=context.user_data.get('drill_context_example') or '',
                example_ru=context.user_data.get('drill_context_example_ru') or '',
            )
        else:
    ...
    if step == 'context' and word.get('word_id'):
...
async def _start_practice_intro(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    words: list[dict],
    *,
    ...
    context.user_data['mode'] = 'daily_intro'
    context.user_data['daily_intro_source'] = source
    context.user_data['daily_intro_queue'] = list(words)
    context.user_data['daily_intro_selected'] = []
    context.user_data['daily_intro_known_count'] = 0
    context.user_data['daily_intro_total'] = len(words)
    await _show_daily_intro_card(update, context)


async def _start_practice_flow(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    from_plan: bool = False,
...
async def start_word_review(update: Update, context: ContextTypes.DEFAULT_TYPE,
                            *, from_plan: bool = False):
    await _start_practice_flow(update, context, from_plan=from_plan)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'diag:start':
        ...
        await _start_practice_flow(update, context)
        ...
        elif word and step == 'context':
            options_words = context.user_data.get('drill_choice_words') or []
            if options_words:
                await _ack_callback(query)
                await _flash_drill_options_hint(
                    context, query.message.chat_id, options_words,
                )
            else:
                await _ack_callback(query)
        elif word:
        ...
        if len(parts) < 5:
            return
        level, category = parts[3], parts[4]
        await _show_rules_bank_page(
            update, context, level=level, page=0, category=category,
        )
